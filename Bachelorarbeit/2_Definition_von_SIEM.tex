...
Auf der Abbildung \ref{fig:Loki-Architektur} empfängt \quotes{distributor}  die Streams von den (1) \glsplural{Endpoint} und überprüft ihre Richtigkeit im Bezug auf den Zeitstempel und Länge der Logzeile. In dem \quotes{distributor} wird auch die Aufnahmenrate konfiguriert auch die Aufnahmenrate. Der \quotes{distributor} leitet die Streams zum (2) \quotes{ingester} weiter, wo dieser die Daten in kleinen Datenblöcken (3) speichert. Diese Daten werden wiederum in der Database (3) je nach \quotes{label} aufgenommen werden. Dieses Verfahren soll Skalierbarkeit und Fehlertoleranz ermöglichen. Schließlich beschäftigt sich der \quotes{querier} mit der Abfragen (4) in der \gls{abfragesprache} \gls{logql}. Der \quotes{querier} holt Daten sowohl von \quotes{ingester} als auch von der Database. Sobald dieses Verfahren fertig ist, kann Grafana \gls{frontend} nach diesen Daten \citep{Grafana_LokiArchitektur} und \citep{Grafana_LokiArchitektur2} fragen.
