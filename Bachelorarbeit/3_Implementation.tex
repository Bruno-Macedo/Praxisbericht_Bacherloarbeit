...
Jedes Angriffszenario hat spezifische Regeln, die mit \gls{logql} aufgebaut sind. In Promtail wird jeder \gls{Endpoint} als \quotes{Instance} bezeichnet. Eine oder mehrere \quotes{Instances} werden einem \quotes{Job} zugewiesen. Diese Struktur stammt aus dem Tool \gls{prometheus}. Die \quotes{Instances} in einem \quotes{Job} werden nach den gleichen Regeln verarbeitet. Die Abfrage für unseren Angriff sieht folgendermaßen aus \citep{VoidQuark_sshlogs}:
...
(1)   Warnmeldungen können in beliebigen Gruppen kategorisiert werden. Diese 
      können von den Nutzern entsprechend ihrer Anforderungen und Bedürfnisse 
      definiert werden.
(2)   Ab diesem Punkt beginnen wir mit der Definition der Regelsätze für die 
      Erkennung von Warnmeldungen. Diese umfassen:
(2.1) Titel für den Alert, zum Beispiel "Potenzieller Angriff gegen SSH"
(2.2) LogQL-Regelsätze für die Erkennung der Warnmeldung, welche die in den 
      vorherigen Schritten definierten Abfragen verwenden.
(2.3) Frei definierbare Metadaten über die Warnmeldung, die für eine weitere
     Analyse oder Kategorisierung nützlich sein können.
\end{Verbatim}
}
...
Im Anhang (siehe \ref{appendix:Warnmeldungskonfiguration}) befindet sich die Konfigurationsdatei für unsere Warnmeldung. Nachdem alles korrekt konfiguriert wurde, haben wir die folgende E-Mail erhalten:
