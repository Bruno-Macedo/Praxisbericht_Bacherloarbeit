...
\begin{framed}
   ...
   14 14:05:30 opfersystem2 \textbf{\textcolor{red}{sshd[}}1698]: \textbf{\textcolor{red}{Failed}} password for \textbf{\textcolor{blue}{administrator}} from \textbf{\textcolor{violet}{10.0.2.15}} port    58036 ssh2

   Diese Elementen werden folgendermaßen gelesen und extrahiert, um die Aufsummierung auszugeben:
   

\begin{Verbatim}
sum by(username) (rate({job="varlogs"} 
Hiermit wird die Aufsummierung der Benutzernamen definiert, die wir mit 
"Patterns" in LogQL definiert haben. "Patterns" ermöglichen die einfache
Extrahierung von Informationen aus einer Zeile.

|= `sshd[`|= `: Failed` 
Regular Expression für die Suche nach Zeilen mit diesen Einträge

|= !~ `invalid user` != `test`
Regular Expression für die Suche nach Zeilen ohne diese Einträge

| pattern `<_> for <username> from <add> port <_>` [$__range]))
Die Definition des Wortes "username" als "Pattern" dient dazu, einen 
Benutzernamen aus der Logdatei zu extrahieren. Die Platzhalter "<_>" 
sind unbenannte Elemente, die in diesem Fall auf die Einträge "password"
und "10.0.2.15" in der Zeile verweisen.
\end{Verbatim}
