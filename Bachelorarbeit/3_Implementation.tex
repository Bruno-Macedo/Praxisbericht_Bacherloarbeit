...
\begin{enumerate}[noitemsep]
   \item Einrichtung von \glsplural{vm} für die Opfersystemen und den Angreifer
...
Für das Opfersystem haben wir uns für die Passwörter \quotes{qwertz} und \quotes{password} entschieden, da sie laut einer Umfrage von \cite{silicon_passwort} zu den zehn am häufigsten verwendeten Passwörtern in Deutschlandgehören gehören. Für die Durchführung des \gls{spraying} haben wir folgende Benutzername-Passwort Kombinationen erstellt:
...
Grafana Loki bietet interne und externe Möglichkeiten Logdateien zu empfangen. Die internen beziehen sich auf Grafana Tools, während die externe unabhängige Methode von Grafana benutzen:
...
In unserer Arbeit verwenden wir \textbf{Promtail}, der in einem \gls{container} läuft. Diese Instanz sendet die von uns ausgewählten Logdateien an Grafana und verarbeitet alle Dateien innerhalb eines sogenannten \quotes{jobs}. Promtail kann Logdateien nur zu Loki oder zu anderen Promtail-Instanz schicken \citep{Grafana_Promtail}. Die Abbildung \ref{fig:Integration_Loki_Promtail_Grafana} auf der Seite \pageref{fig:Integration_Loki_Promtail_Grafana} zeigt diese beschriebene Struktur.
...
\newpage
Die nächste Abbildung, \ref{fig:GrafAgents}, zeigt den Kommunikationfluss zwischen Grafana Agents und die integrierten Tools:
...
Grafana bietet ein Konfigurationsmuster für die Eingabe und Darstellung von \gls{ssh} Events an. In dieser Konfiguration sind bereits Regesätze für die
...
Für jedes Angriffszenario benutzen wir spezifische Regeln, die mit \gls{logql} aufgebaut sind. Die Filterung findet mithilfe von zwei \quotes{Labels} \quotes{instance} und \quotes{job} statt. In Promtail wird jeder \gls{Endpoint} als \quotes{instance} bezeichnet. Eine oder mehrere \quotes{instances} werden einem \quotes{job} zugewiesen. \quotes{Jobs} beziehen sich auf die Vearbeitung des Inhalts der Logdateien nach dem spezifizierten Regeln, in unserem Fall, Überprüfung von \gls{ssh}-Logdateien. Diese Struktur stammt aus dem Tool \gls{prometheus}. Alle unsere \quotes{instance} werden in einem \quotes{job} eingepackt, wo sie nach den gleichen Regeln verarbeitet werden. Zusätzliche \quotes{Labels} können auch definiert werden \citep{Prometheus_JobInstance}. Das folgende Diagramm, \ref{fig:Labels_GrafanaLoki}, stellt die Beziehung zwischen dieser beiden \quotes{Labels} dar:
...
\begin{enumerate}[noitemsep]
   ...
   \item Graphische Darstellung der Ergebnisse in Grafana mit den in Loki verwendeten Regelsätzen
\end{enumerate}
}
...
Für unseren ersten Test erstellen wir Warnmeldungen über die \textbf{\gls{GUI}} von Grafana für fehlgeschlagene Anmeldeversuche für existierenden (A) und nicht existierenden (B) Benutzername. Wenn dieser Wert größer als fünf ist, dann wird eine E-Mail mit der Wanrmeldung geschickt. Wir definierten die oben genannten Elemente definiert (komplete Konfiguration im Anhang \ref{appendix:Warnmedungskonfiguration}) und verwendeten die folgenden Regelsätze  \citep{VoidQuark_sshlogs}. Die generierte Warnmeldung ist auf der Abbildung \ref{fig:ErsteWarnmeldung} dargestellt.
...
|="sshd[" 
|~": Invalid |: nConnection closed by authenticating user |: Failed .* user" 
