...
%    \hline
%    \multicolumn{1}{|c|}{\textbf{\gls{logql}-Code-Schnipsel}} & \multicolumn{1}{|c|}{\textbf{Beschreibung}} \\
%    \hline
%    sum by(add) (rate(\{job=\grqq{}varlogs\grqq{}, \newline instance=~\grqq{}\$instance\grqq{}\} & Hiermit wird die Aufsummierung der Benutzernamen definiert, die wir mit \quotes{Patterns} in \gls{logql} definierten. \quotes{Patterns} ermöglichen die einfache Extrahierung von Informationen aus einer Zeile. Wir holen alle Log-Einträge, die sich auf den Job \quotes{SSH\_LOGS} beziehen. Wir können auch nach spezifischen Endpoint filtern, indem wir das Schlüsselwort \quotes{instance} benutzen.. \\
%    \hline
...
\textbf{\textcolor{red}{Failed}}` !~ `invalid user` !~ `\textit{Legitimer_Nutzer}` !~ `\textit{Legitime_Adresse}` 
